...
import { createSchedule, getSchedules } from "@/lib/api/schedule-helpers";
...
export async function GET(req: Request) {
  ...
  try {
    const { schedules, total } = await getSchedules({
      crew_id,
      user_id,
      start_date,
      end_date,
...
export async function POST(req: Request) {
  ...
  try {
    ...
    // 필수 필드 검증
    if (!title || !event_date || !total_capacity) {
      return NextResponse.json(
        { error: "title, event_date, total_capacity are required" },
        { status: 400 },
      );
    }
