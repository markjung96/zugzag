...
export function EventCalendar({
  ...
  // 에러 상태
  if (error) {
    return <EventCalendarError error={error} onRetry={onRefetch} />;
  }

  // 정상 렌더링
  return <EventCalendarNormal data={data || []} isFetching={isFetching} />;
}

// 정상 캘린더
function EventCalendarNormal({ data, isFetching }: { data: MockEvent[]; isFetching: boolean }) {
  ...
  const calendarDays = useMemo(() => {
    ...
    for (let i = 0; i < 42; i++) {
      ...
      const dayEvents = data.filter((e) => e.event_date === dateStr);
          ...
          {calendarDays.map((day, index) => (
            ...
            >
              ...
              <div className="space-y-1">
                ...
                {day.events.length > 2 && (
                  <div className="px-2 text-xs text-zinc-500">+{day.events.length - 2}개 더</div>
                )}
              </div>
            </div>
          ))}
...
// 에러 상태
function EventCalendarError({ error, onRetry }: { error: Error; onRetry: () => void }) {
  return (
    <motion.div
      initial={{ opacity: 0, y: 20 }}
      animate={{ opacity: 1, y: 0 }}
