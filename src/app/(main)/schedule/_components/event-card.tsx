...
export function EventCard({ event, index = 0 }: EventCardProps) {
  ...
  const hasMultiplePhases = event.phases.length > 1;
            ...
            <div className="mb-4 flex items-start justify-between">
              <div className="flex-1">
                <div className="mb-1 flex items-center gap-2">
                  <span className="text-xs font-medium text-orange-500">{event.crew.name}</span>
                  {getAttendanceBadge()}
                </div>
              ...
              {event.stats && (
                <div className="flex items-center gap-2 text-sm">
                  ...
                  <div className="flex items-center gap-2">
                    <span className={isPast ? "text-zinc-500" : "text-zinc-300"}>
                      {event.stats.attending_count} / {event.total_capacity}명
                    </span>
                    {event.stats.waitlist_count > 0 && (
                      <span className="text-xs text-yellow-500">
                        (대기 {event.stats.waitlist_count})
                      </span>
                    )}
                    {event.stats.checked_in_count > 0 && isPast && (
                      <span className="text-xs text-green-500">
                        <UserCheck className="mr-1 inline h-3 w-3" />
                        {event.stats.checked_in_count}명 참석
                      </span>
                    )}
                  </div>
                </div>
          ...
          {event.stats && !isPast && (
            <div className="h-1 bg-zinc-800">
              <motion.div
                initial={{ width: 0 }}
                animate={{
                  width: `${(event.stats.attending_count / event.total_capacity) * 100}%`,
                }}
