...
export function EventList({
  ...
  // 에러 상태
  if (error) {
    return <EventListError error={error} onRetry={onRefetch} />;
  }

  // 빈 데이터
  if (isEmpty) {
    return <EventListEmpty searchQuery={searchQuery} />;
  }
...
// 에러 상태
function EventListError({ error, onRetry }: { error: Error; onRetry: () => void }) {
  return (
    <motion.div
...
// 빈 데이터
function EventListEmpty({ searchQuery }: { searchQuery?: string }) {
  return (
    <motion.div
      initial={{ opacity: 0, y: 20 }}
      animate={{ opacity: 1, y: 0 }}
