...
export function EditScheduleContent() {
  ...
  const validation = useMemo(() => {
    ...
    const emptyLocationRounds = rounds.filter((r) => !r.location.trim()).map((r) => r.roundNumber);
    if (emptyLocationRounds.length > 0) {
      errors.push(`${emptyLocationRounds.join(", ")}차 일정의 장소를 입력해주세요`);
    }

    const invalidCapacityRounds = rounds.filter((r) => !r.isUnlimited && r.capacity <= 0).map((r) => r.roundNumber);
  ...
  return (
    <div className="flex min-h-[calc(100vh-5rem)] flex-col bg-background">
      ...
      <div className="flex-1 px-4 py-2">
                ...
                <div key={index} className="rounded-xl border border-border bg-card p-4">
                  <div className="flex items-start justify-between">
                    ...
                    <div className="flex items-center gap-1">
                      <Button variant="ghost" size="icon" className="h-8 w-8" onClick={() => openRoundEditor(index)}>
        ...
        {mutation.isError && (
          <div className="mt-2 rounded-lg bg-destructive/10 p-3 text-sm text-destructive">{mutation.error.message}</div>
        )}
      </div>
      ...
      <AlertDialog open={showConfirmDialog} onOpenChange={setShowConfirmDialog}>
        <AlertDialogContent>
          <AlertDialogHeader>
            ...
            <AlertDialogDescription>
              일정을 수정하면 기존 참석/대기 내역이 모두 초기화됩니다. 참석자들은 다시 참석 신청을 해야 합니다.
            </AlertDialogDescription>
          </AlertDialogHeader>
          <AlertDialogFooter>
        ...
        <SheetContent side="bottom" className="h-auto max-h-[85vh] rounded-t-3xl pb-safe">
          ...
          {currentRound && editingRound !== null && (
            <div className="space-y-5 overflow-y-auto px-1 pb-4">
                ...
                <div className="mb-3 flex items-center justify-between">
                  ...
                  <div className="flex items-center gap-2">
                    ...
                    <Switch
                      ...
                      onCheckedChange={(checked) => updateRound(editingRound, { isUnlimited: checked })}
                    />
                  </div>
                </div>
              ...
              <Button className="w-full rounded-xl py-6 text-base font-semibold" onClick={() => setIsSheetOpen(false)}>
                완료
              </Button>
            </div>
          )}
