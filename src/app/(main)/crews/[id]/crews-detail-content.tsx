...
import { useLeaveCrewMutation } from "@/hooks/api/crews/use-leave-crew-mutation";
...
export function CrewsDetailContent() {
  const { id: crewId } = useParams<{ id: string }>();
  const router = useRouter();

  const { copied, copy } = useClipboard();

  const { data: crew, isFetching: crewFetching } = useCrewQuery(crewId);
  const { data: schedulesData } = useCrewSchedulesQuery(crewId);

  const leaveMutation = useLeaveCrewMutation(crewId);
  ...
  return (
    <div className="flex min-h-[calc(100vh-5rem)] flex-col bg-background">
      ...
      <RefetchOverlay isFetching={crewFetching}>
        <div className="flex flex-1 flex-col gap-6 px-4 py-5 pb-24">
          ...
          {!isLeader && (
            <section className="mt-auto pt-4">
              <div className="mb-3 flex items-center gap-2 px-1">
                ...
                <h2 className="text-sm font-semibold text-muted-foreground">크루 탈퇴</h2>
              </div>
              <AlertDialog>
                ...
                <AlertDialogContent className="rounded-2xl">
                  <AlertDialogHeader>
                    <AlertDialogTitle>크루를 탈퇴하시겠습니까?</AlertDialogTitle>
                    <AlertDialogDescription>탈퇴 후에는 초대 코드를 통해 다시 가입해야 합니다.</AlertDialogDescription>
                  </AlertDialogHeader>
                  <AlertDialogFooter>
