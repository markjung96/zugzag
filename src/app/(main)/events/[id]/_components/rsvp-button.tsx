...
type RsvpButtonProps = {
  eventId: string;
  userAttendance?: MockAttendance;
  isFull: boolean;
  allowWaitlist: boolean;
};

export function RsvpButton({ userAttendance, isFull, allowWaitlist }: RsvpButtonProps) {
  ...
  const handleSubmit = async () => {
    ...
    // TODO: API 호출
    console.log("RSVP:", { status: selectedStatus, note: userNote });

    alert(`참석 상태가 "${getStatusLabel(selectedStatus)}"(으)로 변경되었습니다`);
    setShowModal(false);
    setUserNote("");
    // 페이지 리프레시 또는 상태 업데이트
  };
